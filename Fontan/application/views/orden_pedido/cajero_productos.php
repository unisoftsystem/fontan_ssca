    ...
    <?php
        ...
        if(empty($UserName)) { // Recuerda usar corchetes.
            header('Location: ' . base_url() . "index.php/ordenpedido/cajero");
        }   
    ?>
		...
		<div style="margin-top: -25px">
			...
			<table cellpadding="0" cellspacing="0" width="100%">
				<tr>
					...
					<td rowspan="2" valign="top"><img src="<?= base_url() . $this->session->userdata('ImagenFotograficaUsuarioSesionApp');?>" width="70px" style="margin-top: 10px" id="fotoUsuario"/></td>
					...
					.done(function( data ) {
						...
						if($.trim(data) != "[]"){
							...
							var open_panel_count = 0;
								if(json[i].Imagen){
									foto = "<?= base_url();?>" + json[i].Imagen;
								}else{
									foto = "<?= base_url();?>images/box.png";
								}
					...
					if($.trim(data) != "[]" && $.trim(data) != "[][]"){
						...
						$.each(json, function(i, item) {
							...
							if(json[i].Imagen.length != 0){
								fotoProducto = "<?= base_url();?>" + json[i].Imagen;
							}else{
								fotoProducto = "<?= base_url();?>images/box.png";
							}
							var cantiMax = 0;
					...
					if(jsonCarrito[i] != null){
						if((i + 1) % 2 == 1){
							$("#tbody").append('<tr class="even" validrow="true">' +
								...
								'<td><img src="<?= base_url();?>images/stop.png" width="20" id="borrar' + i + '"/></td>' +
							'</tr>');						
						}else{
							$("#tbody").append('<tr class="odd" validrow="true">' +
								...
								'<td>' + jsonCarrito[i].cantidad + '</td>' +
								'<td>' + jsonCarrito[i].total + '</td>' +
								'<td><img src="<?= base_url();?>images/stop.png" width="20" id="borrar' + i + '" iter="' + i + '"/></td>' +
							'</tr>');						
						}

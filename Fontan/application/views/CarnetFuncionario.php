...
<style>
	...
	@font-face {
		...
		src: url(<?= base_url();?>fonts/SEGOEUIB.ttf) format("truetype");
	}
	@font-face {
		font-family: "SegoeUiNormal";
		src: url(<?= base_url();?>fonts/SEGOEUI.ttf) format("truetype");
	}
	.tabla1{
		border-radius:16px;
		width:470px; 
	...
	.paralelogramo {
		...
		background-image:url(<?= base_url();?>images/barra-partea.png);
	...
	.paralelogramo3 {
		width: 150px; 
		height: 35px;  
		background-image:url(<?= base_url();?>images/lobena.png);
		background-size: 100% 100%;
		background-repeat: no-repeat;
		float:right;
		font-style:normal;
	...
	.paralelogramo4 {
		width: 100%; 
		height: 70px;  
		background-image:url(<?= base_url();?>images/barra-parteb.png);
		background-size: 100% 100%;
		background-repeat: no-repeat;
		margin-top:210px;
		z-index:1;
...
<body>
	<div class="tabla1">
	   <div style="width: 145px; height:170px; background-color: #FFF; margin-top:40px; margin-left: 15px; position: relative"><img src="?= base_url();?>images/stock_people.png" width="100%" height="100%" id="foto"/></div>
		
		<div style="position: absolute; margin-top:-190px; float: right; width:300px; /*border: solid #000;*/ margin-left:159px">
			<img src="?= base_url();?>images/LOGO HORIZONTAL AZUL_ALTA.png" width="140" class="logoCarnet"/>
		...
		//if(jsonUsuario.length > 0){
			$.each(jsonUsuario, function(i, item) {
				...
				.done(function( respuesta ) {console.log(respuesta)
					$("#imageQR").attr("src", "<?= base_url();?>" + respuesta);
				});			

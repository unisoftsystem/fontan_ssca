    ...
    <form class="cmxform" method="POST" action="" id="">
      <div class="row" style="margin-top: -20px">
        <div class="col-md-12" style="background-image: url('<?= base_url();?>img/www logo rutas escolares.png'); background-repeat: no-repeat; background-size: 100% 100%; padding-top: 10px; padding-bottom: 10px; height: 192px" align="center">
            ...
            <a style="position: absolute; bottom: -30px; right: 300px; border: 5px solid #f59540; border-radius: 100%; padding: 0px" href="<?= base_url();?>index.php/rutas/editar"><img src="<?= base_url();?>img/www modificarcion ruta escolar.png" width="100px" height="100px" border="0"></a>

            <a style="position: absolute; bottom: -6px; right: 190px" href="<?= base_url();?>index.php/rutas/obtener"><img src="<?= base_url();?>img/www tracking ruta escolar.png" width="100px" height="100px" border="0"></a>
        ...
        <div class="col-md-6" align="center">
          <div style="border: 1px solid #ffe9a3; width: 98%; height: 470px; padding: 10px">
            <form role="form" action="" method="POST">  
              ...
              <div class="row"> 
                <div class="col-md-9">
                  <input type="text" class="form-control" id="busqueda" name="busqueda" placeholder="Busqueda por Apellido" style="margin-top: 5px; background-color: #ffe9a3;" disabled="disabled" disabled="disabled">
                </div>
                <div class="col-md-3">
            ...
            <div style="width: 100%; background: #fff1c5;" align="right">
              <img style="position: absolute; left: 63%; top: 25%" width="150" src="<?= base_url();?>img/B1.png">              
      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <button type="button" id="btnCrearRutaParticuliaridad" class="btn btn-primary pull-right" data-toggle="modal" data-target="#modalParticuliaridad" style="display: none; margin-left: 10px;">
            AGREGAR A PARTICULIARIDADES
          </button>
        
          <button type="button" id="btnCrearRuta" class="btn btn-primary pull-right" style="background-color: #f59540; border-color: #f59540; display: none;"><b>MODIFICAR RUTA ESCOLAR</b></button>
        </div>
      </div>

      <!-- Modal particuliaridades -->
      <div class="modal fade" id="modalParticuliaridad" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
              <h4 class="modal-title" id="myModalLabel">Agregar particuliaridades</h4>
            </div>
            <div class="modal-body">
              <div class="form-group">
                <label>Fecha reemplazo</label>
                <input type="date" id="fecha_reemplazo">
              </div>
              <div class="form-group">
                <label>Observación</label>
                <textarea id="observaciones_particuliaridades" class="form-control"></textarea>
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
              <button id="guardar_particuliaridad" type="button" class="btn btn-primary">Guardar cambios</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <script src="<?= base_url();?>js/jquery.babypaunch.spinner.min.js"></script>
      ...
      <script type="text/javascript">
        ...
        {  
          ...
          if((Date.parse(fech1)) > (Date.parse(fech2))){
            ...
            if($.trim(horaInicio) != "" && $.trim(horaFin) != "" && $.trim(fechaInicio) != ""){
              ...
              $.post("<?= base_url();?>index.php/rutas/listarMonitoresEditar", datos)
              .done(function( data ) {console.log(data)
                ...
                //$("#monitor").val($('#monitores').val())
                $("#monitores").val($('#monitor').val())
              });

              $.post("<?= base_url();?>index.php/rutas/listarVehiculosEditar", datos)
              .done(function( data ) {console.log(data)
                $("#ruta").empty();
                var json = JSON.parse(data);
                $.each(json, function(i, item) {
                ...
                $("#vehiculohidden").val($('#ruta').val())
                MostrarSillas()
              });

              $.post("<?= base_url();?>index.php/rutas/listarConductoresEditar", datos)
              .done(function( data ) {console.log(data)
                $("#conductor").empty();
                var json = JSON.parse(data);
                $.each(json, function(i, item) {
                ...
                //$("#conductor").val($('#conductores').val())
                $("#conductores").val($('#conductor').val())
              });
        ...
        $("#selectruta").change(function(e) {
          ...
          if(ruta != "Seleccione"){
            ...
            .done(function( data ) {
              //console.log(data)
              $("#btnCrearRuta").css({"display":"block"})
              $("#btnCrearRutaParticuliaridad").css({"display":"block"})
              var json = JSON.parse(data);
              ...
              $.each(json, function(i, item) {
                ...
                var datos = {
                  ruta: ruta,
                  horainicial: json[i].horainicial,
                  horafinal: json[i].horafinal,
                  fechainicial: json[i].fechainicial,
                  fechafinal: json[i].fechafinal,
                  repetir: json[i].repetir
                }       
                
                $.post("<?= base_url();?>index.php/rutas/listarMonitoresEditar", datos)
                .done(function( data ) {console.log(data)
                  $("#monitor").empty();
                  var jsonMonitor = JSON.parse(data);
                  $.each(jsonMonitor, function(n, item) {
                    $("#monitor").append('<option value="' + jsonMonitor[n].idmonitor + '">' + jsonMonitor[n].nombre + ' ' + jsonMonitor[n].apellido + '</option>');
                  });
                ...
                .done(function( data ) {console.log(data)
                  $("#ruta").empty();
                  var jsonVehiculos = JSON.parse(data);
                  $.each(jsonVehiculos, function(n, item) {
                    $("#ruta").append('<option value="' + jsonVehiculos[n].idvehiculo + '" sillas="' + jsonVehiculos[n].sillas + '">' + jsonVehiculos[n].categoria + ' ' + jsonVehiculos[n].placa + '</option>');

                    if((jsonVehiculos.length - 1) == n){
                      $("#ruta").val(json[i].idruta)
                      $("#vehiculohidden").val(json[i].idruta)
                      //console.log(json[i].idruta)
                ...
                .done(function( data ) {//console.log(data)
                  $("#conductor").empty();
                  var jsonConductor = JSON.parse(data);
                  $.each(jsonConductor, function(n, item) {
                    $("#conductor").append('<option value="' + jsonConductor[n].idconductor + '">' + jsonConductor[n].nombre + ' ' + jsonConductor[n].apellido + '</option>');
                  });
                  $("#conductor").val(json[i].id_conductor) 
                  $("#conductores").val(json[i].id_conductor) 
                });

                var jsonUsuario = json[i].estudiantes;
                MostrarEstudiantes(jsonUsuario) 
                ...
                $("#radioNunca").removeAttr("disabled"); 
                $("#radioDiariamente").removeAttr("disabled"); 
                $("#radioSemanalmente").removeAttr("disabled"); 
                $("#radioMensualmente").removeAttr("disabled"); 
                $("#conductor").removeAttr("disabled"); 
                $("#monitor").removeAttr("disabled"); 
                $("#btnFiltro").removeAttr("disabled"); 
                $("#pasar").removeAttr("disabled"); 
                $("#pasars").removeAttr("disabled"); 
                ...
                var latLng = new google.maps.LatLng(json[0].latorigen,json[0].longorigen);
               ...
               //función que actualiza los input del formulario con las nuevas latitudes
                ...
                var latLng1 = new google.maps.LatLng(json[0].latdestino,json[0].longdestino);
                ...
                $("#lat").val(json[0].latorigen)
                $("#long").val(json[0].longorigen)
                $("#lats").val(json[0].latdestino)
                $("#longs").val(json[0].longdestino)  
          ...
          if(jsonUsuario.length > 0){
            ...
            $.each(jsonUsuario, function(i, item) {
              if($("#" + jsonUsuario[i].NumeroId).length == 0){
                $("#container-field > div").append('<div class="drag" id="' + jsonUsuario[i].NumeroId + '">' +
                  '<div class="col-md-11" align="center" style="padding: 0px">' +
                    '<img width="58px" height="58px" id="foto' + jsonUsuario[i].NumeroId + '" src="<?= base_url();?>' + jsonUsuario[i].ImagenFotografica + '">' +
                  ...
                  '<div class="col-md-1" style="padding: 0px; margin: 0px">' +
                    '<a class="boxclose" id="back' + jsonUsuario[i].NumeroId + '" document="' + jsonUsuario[i].NumeroId + '" nombre="' + jsonUsuario[i].PrimerNombre + ' ' + jsonUsuario[i].PrimerApellido + '" direccion="' + jsonUsuario[i].Direccion + '">' +
                      '<img src="<?= base_url();?>img/izq.png" width="20">' +
                    '</a>' +
                  '</div>' +
                '</div>')       

                agregarTabla(jsonUsuario[i].PrimerNombre + " " + jsonUsuario[i].PrimerApellido, jsonUsuario[i].Direccion, jsonUsuario[i].NumeroId)               
                  ...
                  switch($('#' + documento + " > .col-md-1 > a > img").attr("src")){
                    case "<?= base_url();?>img/der.png":
                      ...
                      if($("#container-field > div > .drag").length < parseInt($('#ruta option:selected').attr("sillas"))){
                        ...
                        $('#' + documento + ' > .col-md-1 > a > img').attr("src", '<?= base_url();?>img/izq.png')
        ...
        function MostrarSillas() {
          ...
          $.post("<?= base_url();?>index.php/vehiculos/obtenerVehiculoId", {id: $('#ruta').val()})
          .done(function( data ) {console.log(data)
            if($.trim(data) != "{}"){
              var json = JSON.parse(data)
              $("#sillasOcupadas").html($("#container-field > div > .drag").length + "/" + json.sillas)
            }
          });
        }
                ...
                for (var i = 0; i < jsonUsuario.length; i++) {
                  ...
                  if($("#" + jsonUsuario[i].NumeroId).length == 0){
                    ...
                    $("#container-player").append('<div class="drag" id="' + jsonUsuario[i].NumeroId + '">' +
                      ...
                      '<div class="col-md-1" style="padding: 0px; margin: 0px">' +
                        '<a class="boxclose" id="back' + jsonUsuario[i].NumeroId + '" document="' + jsonUsuario[i].NumeroId + '" nombre="' + jsonUsuario[i].PrimerNombre + ' ' + jsonUsuario[i].PrimerApellido + '" direccion="' + jsonUsuario[i].Direccion + '">' +
                          '<img src="<?= base_url();?>img/der.png" width="20">' +
                        '</a>' +
                      '</div>' +
                    '</div>')
        ...
        function mostrarBotonesTabla() {
          $("#tablaOrden > tbody > tr").each(function (index) {
            $(this)[0].children[0].innerHTML = abecedario[index + 1].toUpperCase();
        ...
        function agregarTabla(nombre, direccion, documento) {
          ...
          $("#tablaOrden > tbody").append('<tr id="tr' + documento + '" ind="' + contadorID + '" documento="' + documento + '">' +
            '<td valign="middle" align="center">' + abecedario[contadorID + 1].toUpperCase() + '</td>' +
        ...
        */
        ...
        $("#btnCrearRuta").click(function(e) {
          ...
          if($('#container-field > div').length > 0){
            ...
            $("#btnCrearRuta").attr("disabled", "disabled"); 
            $.post("<?= base_url();?>index.php/rutas/modificar", datos)
            .done(function( data ) {
                console.log(data)
                $("#spin").hide();
                $("#spin").css({"display":"none"})
                alertify.alert("Ruta modificada con exito", function(){
                  window.location.href = "<?= base_url();?>index.php/rutas/editar";
                });
        ...
        // ===============================
        //  GUARDAR PARTICULIARIDAD
        // ===============================
        observaciones_particuliaridades
        guardar_particuliaridad
        $("#guardar_particuliaridad").click(function(e) {
          ...
          var fechaFin = $("#txtFechaFinal").val();
          var repetir =  $('input:radio[name=radioRepetir]:checked').val();
          var observaciones_particuliaridades = $("#observaciones_particuliaridades").val();
          var fecha_reemplazo = $("#fecha_reemplazo").val();
          var estudiantes = obtenerEstudiantesAgregados();
          ...
          if($('#container-field > div').length > 0){
            ...
            var datos = {
              ...
              fechainicial: fechaInicio,
              fechafinal: fechaFin,
              observaciones: observaciones_particuliaridades,
              fecha_reemplazo: fecha_reemplazo,
              id_asignacionruta: idruta,
              isParticuliaridad: 1,
            }
            console.log($('#container-field > div').length)
            console.log(obtenerEstudiantesAgregados())            
            ...
            .done(function( data ) {
                ...
                $("#spin").hide();
                $("#spin").css({"display":"none"})
                alertify.alert("Ruta modificada con exito (Particuliaridad)", function(){
                  window.location.href = "<?= base_url();?>index.php/rutas/editar";
                });

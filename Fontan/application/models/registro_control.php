			...
			switch ($_POST["tipo"]) {
				...
				case 'FUNCIONARIO':
					...
					if($data_funcionarios != null){
								...
								foreach ($resultado->result() as $value) {	
									...
									switch ($value->Tipo) {
										case 'ENTRADA':
											...
											$key->Observacion = $value->Observacion;	
						...
						echo json_encode($array_funcionario->result());
					}else{
						echo "[]";
					}
					break;
		...
		if($_REQUEST["numeroID"] == ""){
			...
			switch ($_REQUEST["tipo"]) {
				...
				case 'FUNCIONARIO':
					...
					if($data_funcionarios != null){
						foreach ($data_funcionarios->result() as $key) {	
							$data_resultado = $this->registro_control_model->ReporteConDoc($_REQUEST["fechaInicial"], $_REQUEST["fechaFinal"], $_REQUEST["tipo"], $key->NumeroId);
							if($data_resultado != null){
								foreach ($data_resultado->result() as $value) {	
									$horaRegistro = strtotime( $value->Hora );	
									$horaInicio = strtotime( "07:00:00" );	

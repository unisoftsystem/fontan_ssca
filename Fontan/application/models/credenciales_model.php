...
class Credenciales_model extends CI_Model
{
	...
	function ConsultarUsuarioPorId($usuario){
		$contador = 1;
		$json = "";
		$data = $this->db->query("SELECT u.*, c.*, cre.*, TIMESTAMPDIFF(YEAR, u.fechanacimiento, CURDATE()) AS edad FROM usuarios u INNER JOIN cursos c on c.id = u.curso  INNER JOIN credenciales cre ON u.idUsuario = cre.idUsuarioSecundario WHERE idUsuario='$usuario'");
		...
		if($data->num_rows() > 0){			
			...
			$datosFuncionario = $this->db->query("SELECT u.*, cre.*, '' AS Descripcion, TIMESTAMPDIFF(YEAR, u.fechanacimiento, CURDATE()) AS edad FROM usuarios u INNER JOIN credenciales cre ON u.idUsuario = cre.idUsuarioSecundario WHERE idUsuario='$usuario'");
	...
	function ConsultarUsuarioPorCredencial($idUsuario){
		$json = "";
		$contador = 1;
		$data = $this->db->query("SELECT c.* , u.*, curso.*, TIMESTAMPDIFF(YEAR, u.fechanacimiento, CURDATE()) AS edad FROM `credenciales` c INNER JOIN usuarios u ON u.idUsuario = c.idUsuarioSecundario INNER JOIN cursos curso on curso.id = u.curso WHERE c.`idCredencial` ='$idUsuario'");
		
		if($data->num_rows() > 0){			
			foreach ($data->result() as $value) {				
				$value->clave = base64_decode($value->Clave);

				$datosRestriccion = $this->db->query("SELECT `Log` FROM `restriccion` WHERE `Tipo`='PORVALOR' AND `idEstudiante`='$idUsuario'");
			...
			$dataFuncio = $this->db->query("SELECT c.* , u.*, '' AS Descripcion FROM `credenciales` c INNER JOIN usuarios u ON u.idUsuario = c.idUsuarioSecundario WHERE c.`idCredencial` ='$idUsuario'");
	...
	function ObtenerSaldoActual($idCredencial){
		$SaldoCredencial = 0;
		$datos = $this->db->query("SELECT `SaldoCredencial` FROM `credenciales` WHERE `idCredencial`='$idCredencial' AND `EstadoCredencial`='ACTIVO'");
	...
	function CambiarSaldo($idCredencial, $valor){
		$datos = $this->db->query("UPDATE `credenciales` SET `SaldoCredencial`= '$valor', `EstadoCredencial`='ACTIVO' WHERE `idCredencial`='$idCredencial'");
	}

	//Funcion para cambiar el saldo actual de una credencial
	function CambiarAcudiente($idAcudiente, $idusuario, $idCredencial){
		$this->db->query("UPDATE `credenciales` SET `idUsuarioPrincipal`= '$idAcudiente', `idUsuarioSecundario`='$idusuario' WHERE `idCredencial` = '$idCredencial'");
		$this->db->query("UPDATE `restriccion` SET `idAcudiente`= '$idAcudiente' WHERE `idEstudiante` = '$idCredencial'");
	}

	//Funcion para cambiar el saldo actual de una credencial
	function CambiaridUsuarioLogIn($idusuarioAntiguo, $idusuarioNuevo, $origen){
		$datos = $this->db->query("UPDATE `Log_inventario` SET `session`='$idusuarioNuevo' WHERE `origen`='$origen' AND `session`='$idusuarioAntiguo'");
	}

	//Funcion para cambiar el saldo actual de una credencial
	function CambiarFechaVencimiento($fechaVencimiento, $idusuario){
		$datos = $this->db->query("UPDATE `credenciales` SET `FechaVencimiento`='$fechaVencimiento' WHERE `idUsuarioSecundario`='$idusuario'");
	}

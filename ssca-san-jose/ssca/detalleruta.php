...
<?php
    ...
    $result = mysql_query("SELECT * FROM ruta where idruta='".$valor."'");

    while($row = mysql_fetch_assoc($result))
   {


    echo " Identificacion Ruta: {$row['idruta']}  <br> ".
         "Numero de Placa: {$row['placa']} <br> ".
         "Nombre de Ruta: {$row['nombre_ruta']} <br> ".
         "Identificacion de Monitor: {$row['monitor_idmonitor']} <br> ".
         "Identificacion de Conductor: {$row['conductor_idconductor']} <br> ".
         "--------------------------------<br>";

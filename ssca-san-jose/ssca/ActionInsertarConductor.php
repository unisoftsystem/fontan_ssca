<?php 
	...
	*/
	$dataCredenciales->setIdUsuarioPrincipal($idconductor);	
	$dataCredenciales->setIdUsuarioSecundario($idconductor);
	...
	*/
	...
	$resultUsuarioNuevo = $Controller_Conductor->CrearConductor($dataUsuario);	
	
	$dataMovimientoTarjeta = new DataMovimientos();
	
	$dataMovimientoTarjeta->setIdUsuario($idconductor);	
	...
	echo '[{"resultado":"' . $resultCredenciales . '", "credencial":"' . $resultCredenciales . '", "nombre":"' . $nombre . '", "apellido":"' . $apellido . '", "tipo":"' . strtoupper($TipoUsuario) . '", "numeroId":"' . $idconductor . '", "foto":"' . $file . '", "fechaVencimiento":"' . $mes . '", "arl":"' . $arl . '", "tipoSangre":"' . $tipoSangre . '"}]';

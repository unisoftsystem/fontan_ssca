...
$conexion = $conexionBD->conectar();
 ...
 $registros = mysqli_num_rows ($resultado);
  ...
  while ($registro = mysqli_fetch_array($resultado, MYSQL_ASSOC)){
    ...
    $objPHPExcel->setActiveSheetIndex(0)
          ->setCellValue('F'.$i, $registro["FechaMovimiento"]);
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Reporte de Caja');
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
$objPHPExcel->setActiveSheetIndex(0);
// Redirect output to a client’s web browser (Excel5)
$nombreArchivo = "Reporte_Caja" . date("Y") . date("m") . date("d") . date("H") . date("i") . date("s") . date("u") . ".xls";

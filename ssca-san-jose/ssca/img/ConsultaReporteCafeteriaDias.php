...
<html>
  ...
  <tbody>
    <?php
      ...
      $consulta5="SELECT  DISTINCT productos.*, categoria.Nombre AS NombreCategoria, `sub-categoria`.Nombre AS NombreSubCategoria FROM `productos` INNER JOIN movimientos ON movimientos.FechaMovimiento BETWEEN '$fechai' AND '$fechaf' INNER JOIN  Detalle_OrdenPedido ON Detalle_OrdenPedido.codigoProducto = productos.`codigoProducto` AND movimientos.`DescripcionMovimiento` LIKE CONCAT('%No. Pedido ',Detalle_OrdenPedido.idOrdenPedido, '%') INNER JOIN categoria ON categoria.codigo = productos.Categoria INNER JOIN `sub-categoria` ON `sub-categoria`.codigo = productos.Subcategoria ORDER BY productos.NombreProducto ASC";
      ...
      while($row = mysql_fetch_array($consulta5, MYSQL_ASSOC)){
	      ...
	      $consulta="SELECT SUM(Detalle_OrdenPedido.`cantidad`) AS CantidadTotal, SUM(Detalle_OrdenPedido.`total`) AS Total FROM  `Detalle_OrdenPedido` INNER JOIN movimientos ON movimientos.FechaMovimiento BETWEEN  '$fechai' AND  '$fechaf' AND movimientos.`DescripcionMovimiento` LIKE CONCAT(  '%No. Pedido ', Detalle_OrdenPedido.idOrdenPedido,  '%' ) WHERE  `codigoProducto` =  '$codigoProducto'";
			...
			$registroTotal = mysql_fetch_array($consulta, MYSQL_ASSOC);
 			$cantidadtotal = $registroTotal["CantidadTotal"];
 			$subtotal =  $registroTotal["Total"];
	      //muestro los datos en pantalla
	      echo "<tr><td><center>".$cantidadtotal."</center></td>";

...
class Credenciales extends CI_Controller {
	...
	//Mostrar la vista de visualizar el qr de un usuario
	function layout_qr(){
		$this->load->view('layout_qr');
	}
	...
	function procesoRecaudo(){
		...
		if($validacion > 0){
			...
			$this->load->view('credenciales/procesoRecaudo', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
	...
	function devolucionSaldo(){
		...
		if($validacion > 0){
			...
			$this->load->view('credenciales/devolucionSaldo', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
			echo "<script>window.location.href = '" . base_url() . "index.php/usuarios_sistema/homeInternoModulos';</script>";//Se redirecciona a la pagina del home cuando el usuario que ha iniciado sesion no tiene permiso sobre la pagina que intenta ingresar
		}
	}
	...
	function reporteRecaudo(){
		...
		if($validacion > 0){
			...
			$this->load->view('credenciales/reporteRecaudo', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
			echo "<script>window.location.href = '" . base_url() . "index.php/usuarios_sistema/homeInternoModulos';</script>";//Se redirecciona a la pagina del home cuando el usuario que ha iniciado sesion no tiene permiso sobre la pagina que intenta ingresar
		}
	}
	...
	function reporteDevoluciones(){
		...
		if($validacion > 0){
			...
			$this->load->view('credenciales/reporteDevoluciones', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
			echo "<script>window.location.href = '" . base_url() . "index.php/usuarios_sistema/homeInternoModulos';</script>";//Se redirecciona a la pagina del home cuando el usuario que ha iniciado sesion no tiene permiso sobre la pagina que intenta ingresar
		}
	}
	...
	function reporteReversionPedidos(){
		...
		if($validacion > 0){
			...
			$this->load->view('credenciales/reporteReversionPedidos', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
			echo "<script>window.location.href = '" . base_url() . "index.php/usuarios_sistema/homeInternoModulos';</script>";//Se redirecciona a la pagina del home cuando el usuario que ha iniciado sesion no tiene permiso sobre la pagina que intenta ingresar
		}
	}
	...
	function reportePermisosSalidas(){
		...
		if($validacion > 0){
			...
			$this->load->view('credenciales/reportePermisosSalidas', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
			echo "<script>window.location.href = '" . base_url() . "index.php/usuarios_sistema/homeInternoModulos';</script>";//Se redirecciona a la pagina del home cuando el usuario que ha iniciado sesion no tiene permiso sobre la pagina que intenta ingresar
		}
	}
	...
	function reemplazarCredencial(){
		...
		if($validacion > 0){
			...
			$this->load->view('credenciales/reemplazarCredencial', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
			echo "<script>window.location.href = '" . base_url() . "index.php/usuarios_sistema/homeInternoModulos';</script>";//Se redirecciona a la pagina del home cuando el usuario que ha iniciado sesion no tiene permiso sobre la pagina que intenta ingresar
		}
	}
	...
	//Mostrar la vista de reemplazo de credenciales
	function reimprimirQr(){
    	$this->load->library('logo_footer');
		//Se valida si la pagina a acceder esta dentro de los pemisos del usuario que ha iniciado sesion
		$validacion = $this->permisos_usuarios_sistema_model->verificarPagina("../credenciales/reimprimirQr", $this->session->userdata('UserIDInternoSSCA'));	
		...
		if($validacion > 0){
			...
			//Se cargan los menus de los modulos, submodulos y servicios que estan en el controller services
			$this->load->library('../controllers/services');
			$data['titulo'] = "Reimprimir QR";//Titulo de la pagina, se lo envio al archivo donde esta el header
			$data['menuPrincipal'] = $this->services->menuModulos($this->session->userdata('UserIDInternoSSCA'), $this->session->userdata('SegmentoTextoModulo'));	
			$data['menuServicios'] = $this->services->menuServicios($this->session->userdata('UserIDInternoSSCA'), "", $this->session->userdata('SegmentoCodigoModulo'));
			$this->load->view('header', $data);//Se muestra en el navegador primero el header con titulo y demas cosas agregada
			$logo_footer = $this->logo_footer->obtener_logo();
			...
			$this->load->view('credenciales/reimprimirqr', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
			echo "<script>window.location.href = '" . base_url() . "index.php/usuarios_sistema/homeInternoModulos';</script>";//Se redirecciona a la pagina del home cuando el usuario que ha iniciado sesion no tiene permiso sobre la pagina que intenta ingresar
		}
	}
	...
	function asignarPermiso(){
		...
		if($validacion > 0){
			...
			$this->load->view('credenciales/asignarPermiso', $array_page);//Se carga la vista que esta dentro de la carpeta
		}else{
			echo "<script>window.location.href = '" . base_url() . "index.php/usuarios_sistema/homeInternoModulos';</script>";//Se redirecciona a la pagina del home cuando el usuario que ha iniciado sesion no tiene permiso sobre la pagina que intenta ingresar
		}
	}
	...
	function generarQR(){
		...
		*/
	    ...
	    $this->load->file(getcwd() . DIRECTORY_SEPARATOR . 'plugins/phpqrcode/qrlib.php'); 
	...
	//Funcion para buscar el qr de la credencial del usuario consultado
	function obtener_qr_usuario(){
		//Se consulta el usuario por su idUsuario
		$resultadoPorCredencial = $this->credenciales_model->ConsultarUsuarioPorId($_POST["usuario"]);//Se llama a la funcion de que esta en modelo y el resultado se guarda
		echo $resultadoPorCredencial;
	}

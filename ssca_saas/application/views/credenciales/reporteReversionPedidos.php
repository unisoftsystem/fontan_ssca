    ...
    <?= $footer;?>
      ...
      $.validator.setDefaults({
        ...
        submitHandler: function() {
          var usuarioSeleccionado = $("#txtUserId").val();
        ...
        $.post("<?= base_url();?>index.php/movimientos/GenerarReporteReversionPedidos", datos)
        .done(function( data ) {
          ...
          if($.trim(data) != "[]"){
            ...
            $.each(jsonUsuario, function(i, item) {
              ...
              var fechas = jsonUsuario[i].FechaCancelacion.split("-");
              ...
              if((i + 1) % 2 == 1){
                ...
                '<td>' + fechaDato + '</td>' +
                '<td>' + jsonUsuario[i].HoraCancelacion + '</td>' +
                '<td>' + jsonUsuario[i].idPedido + '</td>' +
                '<td>' + jsonUsuario[i].Productos + '</td>' +
                '<td>' + jsonUsuario[i].Total + '</td>' +
                '<td>' + jsonUsuario[i].OrigenPedido + '</td>' +
                '</tr>';            
      ...
      (function() {
        ...
        $("#commentForm").validate({
         ...
         $("#btnExportarReporte").click(function(e) {
          ...
          var fechaInicial = $("#dateFechaInicial").val();
          var fechaFinal = $("#dateFechaFinal").val();
          var win = window.open("<?= base_url();?>index.php/movimientos/ExportarReversionPedidos?usuario=" + usuarioSeleccionado + "&fechaInicial=" + fechaInicial + "&fechaFinal=" + fechaFinal, '_blank');
          win.focus(); 

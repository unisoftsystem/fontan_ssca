	...
	<script>
		...
		$(document).ready(function(){
			$('#reader').html5_qrcode(function(data){
					...
					console.log(data)
					if(contador < 1){
						$.post("../ordenpedido/GenerarAlmuerzo", {usuario: data, usuarioSesion: usuarioSesion})
						.done(function( result ) {console.log(result)
							
							if($.trim(result) != "{}"){
								var json = JSON.parse("[" + result + "]");						 	
	                  			$("#foto").attr("src", "<?= base_url(1);?>" + json[0].ImagenFotografica);
        ...
        <script type="text/javascript" src="<?= base_url(1);?>js/chat.js"></script>
       	<script>
			...
			$("#OpcionSalir").click(function(e){
                ...
                if(confirmar){
                    $.post("../usuarios_sistema/cerrarSesionUsuarioInterno", {})
                    .done(function( data ) {

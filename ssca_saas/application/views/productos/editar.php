          ...
          <form class="cmxform" method="POST" action="" id="commentForm">
            <div class="row">
              <div class="col-md-8">  
                <div class="row">
                  <div class="col-md-4">  
                      <label for="selectProducto"><font color="#09C" size="2">Nombre del Producto: </font></label>
                  </div>
                  <div class="col-md-8">  
                    <select name="selectProducto" id="selectProducto" class="form-control">
                      ...
                      ?>
                      <option value="<?= $value->codigoProducto?>"><?= $value->NombreProducto;?></option>
                      <?php
    ...
    <?= $footer;?>
      ...
      $.validator.setDefaults({
        ...
        $("#btnGuardar").attr("disabled", "disabled")
          $.post("<?= base_url();?>index.php/producto/modificar", datos)
          .done(function( data ) {console.log(data)
            alert("¡Se actualizo con exito el producto!")
            window.location.href = "<?= base_url();?>index.php/producto/editar";
          });
     ...
     $("#selectProducto").change(function(e) {
      ...
      if($("#selectProducto").val() != "Seleccione"){
        var datos = {
        ...
        $.post("<?= base_url();?>index.php/producto/ObtenerStock", datos)
        .done(function( data ) {
          ...
          if($.trim(data) != "[]"){
            ...
            $.each(json, function(i, item) {         
              $("#txtCodigo").val(json[i].codigoProducto)
              $("#txtNombre").val(json[i].NombreProducto)
              $("#txtDescripcion").val(json[i].Descripcion)
              $("#txtValor").val(json[i].ValorUnitario)
              $("#txtTiempo").val(json[i].hora_maxima)
              $("#txtTiempoC").val(json[i].tiempo_cancelacion)
              $("#txtEdad").val(json[i].edad_max);
              $("#txtEdadMinima").val(json[i].edad)
              $("#selectCategoria").val(json[i].Categoria) 
              listarSubCategorias(json[i].Subcategoria) 
              if(json[i].Estado == "ACTIVO"){
                $("#radioEstadoActivo").prop("checked", true);  // para poner la marca
              }else{
                $("#radioEstadoInactivo").prop("checked", true);  // para poner la marca
              }
              $("#txtStock").val(json[i].Stock)
              $("#imageFoto").attr("src","<?= base_url(1);?>" + json[i].Imagen);
              $("#UrlImagenOculta").val(json[i].Imagen)
            });     

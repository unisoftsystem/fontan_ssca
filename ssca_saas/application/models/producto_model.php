...
class Producto_model extends CI_Model
{
	...
	function ListarProductosSubCategoria($subcategoria, $dia, $idUsuario, $idCredencial){
		...
		$contador = 1;
		if($subcategoria == "53"){
			$dataCambioMenu = $this->db->query("
				SELECT dm . * , m . * , p.Descripcion AS Proteina 
				FROM  `detallenenudia` dm 
				INNER JOIN proteinas p ON p.id = dm.`idProteina` 
				INNER JOIN  `menudia` m ON m.id = dm.idMenuDia 
				WHERE dm.`idUsuario` =  '$idUsuario' 
				
				AND dm.`Dia`=$dia 
				ORDER BY dm.`fecha` DESC , dm.`hora` DESC LIMIT 1
			");

			if($dataCambioMenu->num_rows() > 0){
				$dataProductoCambioMenu = $this->db->query("
					SELECT p.* 
					FROM `productos` p 
					...
					AND (SELECT COUNT(*) FROM restriccion r WHERE r.Log = p.`codigoProducto` AND r.Tipo = 'PORPRODUCTO' AND r.idEstudiante = '$idCredencial') = 0 
				...
				if($dataMenu->num_rows() > 0){
					$dataProductoMenu = $this->db->query("
						SELECT p.* 
						FROM `productos` p 
						WHERE p.`Subcategoria`='$subcategoria' 
						AND p.`Estado`='ACTIVO' 
			...
			$dataProducto = $this->db->query("
				SELECT productos.* 
				FROM `productos` 
				WHERE productos.`Subcategoria`='$subcategoria' 
			...
			if($dataProducto->num_rows() > 0){
				foreach ($dataProducto->result() as $value) {
					...
					$dataRestriccion = $this->db->query("
						SELECT COUNT(*) AS filas 
						FROM restriccion r 
						WHERE r.Log = '$codigoProducto' 
						
						AND r.Tipo = 'PORPRODUCTO' 
						AND r.idEstudiante = '$idCredencial'
					");
					$estadoRestriccion = "";
	...
	function ListarProductos(){		
		$data = $this->db->query("
			SELECT productos.*, categoria.Nombre AS NombreCategoria, `sub-categoria`.Nombre AS NombreSubCategoria 
			FROM productos 

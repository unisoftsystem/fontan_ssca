...
class Ordenpedido_model extends CI_Model
{
	...
	function ConsultaReporteCafeteriaDias($fechai, $fechaf){
		...
		if($data->num_rows() > 0){
			foreach ($data->result() as $valueMovimiento) {
				...
				$dataDetalleOrden = $this->db->query("
					SELECT * 
					FROM `Detalle_OrdenPedido` 
					WHERE `idOrdenPedido`='$codigoPedido'
					
				");

				foreach ($dataDetalleOrden->result() as $valorDetalle) {
					$dataProductos = $this->db->query("
						SELECT 
							productos.*, 
							categoria.Nombre AS NombreCategoria, 
							`sub-categoria`.Nombre AS NombreSubCategoria 
						FROM `productos` 
						INNER JOIN categoria ON categoria.codigo = productos.Categoria 
						INNER JOIN `sub-categoria` ON `sub-categoria`.codigo = productos.Subcategoria
						 
						AND productos.`codigoProducto`='" . $valorDetalle->codigoProducto . "'");
	...
	function PedidosaRevertir($numeroId){
		$datos = $this->db->query("
			...
			FROM ordenpedido 
			INNER JOIN usuarios ON usuarios.idUsuario =  ordenpedido.idUsuario INNER JOIN movimientos ON movimientos.`DescripcionMovimiento` LIKE CONCAT ('%No. Pedido ', ordenpedido.`ConsecutivoInterno`, '%')  AND movimientos.`FechaMovimiento` = CURDATE() WHERE usuarios.NumeroId = '$numeroId' AND ordenpedido.FechaEntrega = '0000-00-00'");
	...
	function PedidosaRevertirPlanificados($numeroId){
		$datos = $this->db->query("
			SELECT 
				...
				usuarios.SegundoApellido 
			FROM ordenpedido 
			INNER JOIN usuarios ON usuarios.idUsuario =  ordenpedido.idUsuario 
			INNER JOIN movimientos ON movimientos.`DescripcionMovimiento` 
			LIKE CONCAT ('%No. Pedido ', ordenpedido.ConsecutivoInterno, '%') 
			WHERE usuarios.NumeroId = '$numeroId' 
			AND ordenpedido.FechaEntrega = CURDATE()

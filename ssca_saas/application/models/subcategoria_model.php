...
class Subcategoria_model extends CI_Model
{
	...
	function crear($datos){		
		...
		$this->db->insert('sub-categoria', $array);//Se llama a la funcion de insertar datos y se le pasa el array asociativo			
	...
	function editar($datos){		
		...
		$this->db->update('sub-categoria', $array);//Se llama a la funcion de insertar datos y se le pasa el array asociativo			
		
	}
	//Funcion para listar todos las bodegas
	function listar($idCategoria){
		$data = $this->db->query("SELECT * FROM `sub-categoria` WHERE `idCategoria`='$idCategoria' ORDER BY `Nombre` ASC");

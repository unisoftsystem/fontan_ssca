...
class Rutas_model extends CI_Model
{
	...
	function get_recorrido_dia($id_asignacionruta, $fecha_reemplazo){
	...
	function listarMensajesCoordinadorAcudiente(){
		...
		$data = $this->db->query("SELECT CONCAT(acudiente.PrimerNombre, ' ', acudiente.SegundoNombre, ' ', acudiente.PrimerApellido, ' ', acudiente.SegundoApellido) AS NombreAcudiente, log_ruta.*, '' AS nombreruta, estudiante.`idUsuario`, estudiante.`TipoId`, estudiante.`NumeroId`, estudiante.`PrimerApellido`, estudiante.`SegundoApellido`, estudiante.`PrimerNombre`, estudiante.`SegundoNombre`, estudiante.`ImagenFotografica`, estudiante.`Direccion`, estudiante.`Telefono1`, estudiante.`Telefono2`, estudiante.`Estado` FROM log_ruta INNER JOIN usuarios_sistema ON usuarios_sistema.idUsuario = log_ruta.`session` INNER JOIN usuarios estudiante ON estudiante.idUsuario = log_ruta.`idestudiante` INNER JOIN usuarios acudiente ON acudiente.idUsuario = estudiante.`idAcudiente` WHERE log_ruta.`tipo`='MENSAJEAACUDIENTESPORESTUDIANTE' UNION SELECT '' AS NombreAcudiente, log_ruta.*, asignacionruta.nombreruta, usuarios.`idUsuario`, usuarios.`TipoId`, usuarios.`NumeroId`, usuarios.`PrimerApellido`, usuarios.`SegundoApellido`, usuarios.`PrimerNombre`, usuarios.`SegundoNombre`, usuarios.`ImagenFotografica`, usuarios.`Direccion`, usuarios.`Telefono1`, usuarios.`Telefono2`, usuarios.`Estado` FROM `log_ruta` INNER JOIN asignacionruta ON asignacionruta.id = log_ruta.`idruta` INNER JOIN usuarios ON usuarios.idUsuario = log_ruta.`idestudiante` WHERE log_ruta.`tipo`='MENSAJEAACUDIENTESPORRUTA'");		
		if($data->num_rows() > 0) return $data;
		else return false;
	}
	...
	function listarMensajesCoordinadorAcudienteFecha($idruta, $fecha, $idUsuario){
		$data = $this->db->query("SELECT CONCAT(acudiente.PrimerNombre, ' ', acudiente.SegundoNombre, ' ', acudiente.PrimerApellido, ' ', acudiente.SegundoApellido) AS NombreAcudiente, log_ruta.*, '' AS nombreruta, estudiante.`idUsuario`, estudiante.`TipoId`, estudiante.`NumeroId`, estudiante.`PrimerApellido`, estudiante.`SegundoApellido`, estudiante.`PrimerNombre`, estudiante.`SegundoNombre`, estudiante.`ImagenFotografica`, estudiante.`Direccion`, estudiante.`Telefono1`, estudiante.`Telefono2`, estudiante.`Estado` FROM log_ruta INNER JOIN usuarios_sistema ON usuarios_sistema.idUsuario = log_ruta.`session` INNER JOIN usuarios estudiante ON estudiante.idUsuario = log_ruta.`idestudiante` AND estudiante.idUsuario = '$idUsuario' INNER JOIN usuarios acudiente ON acudiente.idUsuario = estudiante.`idAcudiente` WHERE log_ruta.`tipo`='MENSAJEAACUDIENTESPORESTUDIANTE' AND log_ruta.`fecha` = '$fecha' UNION SELECT '' AS NombreAcudiente, log_ruta.*, asignacionruta.nombreruta, usuarios.`idUsuario`, usuarios.`TipoId`, usuarios.`NumeroId`, usuarios.`PrimerApellido`, usuarios.`SegundoApellido`, usuarios.`PrimerNombre`, usuarios.`SegundoNombre`, usuarios.`ImagenFotografica`, usuarios.`Direccion`, usuarios.`Telefono1`, usuarios.`Telefono2`, usuarios.`Estado` FROM `log_ruta` INNER JOIN asignacionruta ON asignacionruta.id = log_ruta.`idruta` INNER JOIN usuarios ON usuarios.idUsuario = log_ruta.`idestudiante` AND usuarios.idUsuario = '$idUsuario' WHERE log_ruta.`tipo`='MENSAJEAACUDIENTESPORRUTA' AND log_ruta.`fecha` = '$fecha'");		
		if($data->num_rows() > 0) return $data;
		else return false;
	}
}	
...
class Conductor_model extends CI_Model
{
	...
	function getConductorDoc($idconductor){
		$data = $this->db->query("
			...
			FROM `usuarios` 
			INNER JOIN conductor ON conductor.idconductor = usuarios.NumeroId 
			INNER JOIN credenciales ON credenciales.idUsuarioSecundario = usuarios.idUsuario 
			WHERE conductor.`idconductor`='$idconductor'
	...
	function listar(){
		$data = $this->db->query("
			SELECT conductor.* FROM conductor 
			WHERE (
				SELECT COUNT(*) FROM `asignacionruta` 
				WHERE asignacionruta.`id_conductor` = conductor.idconductor
			) = 0
	...
	function ValidarHoras($hora_inicio, $hora_final, $fecha, $idconductor){
		$estado = true;
  		...
  		$query_select_existe = "
  			...
  			INNER JOIN conductor ON datoscalendario.id_conductor = conductor.idconductor 
  			WHERE (datoscalendario.`fecha` = '$fecha') 
  			AND (datoscalendario.horainicial = '$hora_inicio' 
  			AND datoscalendario.horafinal = '$hora_final') 
  			AND datoscalendario.id_conductor = '$idconductor'

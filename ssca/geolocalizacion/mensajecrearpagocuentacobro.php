...
<html>
    ...
    /* seleccion de datos de tercero para asignacion a pdf */
     $result4 = mysql_query("SELECT  nombre, apellido, area, documento, descuento1, descuento2, descuento3, descuento4, telefono FROM tercero where nombre='".$valor4."'");    
  ...
  <?php 
    /* seleccion de datos de tercero para asignacion a pdf */
     $result5 = mysql_query("SELECT  valor_cuenta FROM cuenta_cobro where idcuenta='".$valor."'");    
  ...
  ?>
    <?php
        ...
        /* seleccionamos el tipo, estilo y tamaño de la letra a utilizar */
            ...
            $pdf->Ln(12);
            $pdf->Ln();
            $pdf->Write (7,"                                     RECIBO DE PAGO No  ".$idpagocuentacobro);
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Write (7,"TERCERO: ".$GLOBALS['nombre'].' '.$GLOBALS['apellido']);
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Write (7,"CC: ".$GLOBALS['documento']);
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Write (7,"TELEFONO: ".$GLOBALS['telefono']);
            $pdf->Ln(); //salto de linea
            $pdf->Ln(); //salto de linea
            $pdf->Ln(); //salto de linea
            $pdf->Write (7,"                                      DESCRIPCION VALOR");
            $pdf->Ln(); //salto de linea
            $pdf->Ln(); //salto de linea
            $pdf->Ln(); //salto de linea
            $pdf->Write (7,"CANCELACION CUENTA COBRO No ".$GLOBALS['valor'].'   Valor  $'.$GLOBALS['valorcuenta']);
            $pdf->Ln();
            $pdf->Ln();
            ...
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Write (7,"VALOR TOTAL A PAGAR $ ".$GLOBALS['valor2']);
